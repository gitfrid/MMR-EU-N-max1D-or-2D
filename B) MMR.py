...
for country in common_countries:
    ...
    if vac_slope < 0 and cases_slope > 0:
        ...
        trace_reported_cases = go.Scatter(
            ...
            mode='lines',  # Use lines only, no markers
            name=f'{legend_name} - reported confirmed cases /1M',  # Use the truncated name here
            yaxis='y2'
        )
...
layout = go.Layout(
    title=f"Measles vaccination coverage  max(1st or 2nd Dose) vs reported confirmed cases /1M for different countries<br>{slope_text} {year_range_text}",
...
fig.write_html(fr"C:\MMR-EU-N-max1D-or-2D\B) MMR {slope_text} {year_range_text}.html")
