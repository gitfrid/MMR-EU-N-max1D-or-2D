...
filename_plot_text = "Dowhy causal estimate on mean vac coverage and cases per 1M Measles"
...
log_file_path = fr"C:\MMR-EU-N-max1D-or-2D\C) {filename_plot_text} valid years for dowhy calc {year_range_text}.txt"
...
fig.add_trace(go.Scatter(
    ...
    y=causal_effects,
    mode='markers',
    name='Causal Effect Vac Coverage on confirmed reported Cases/1M',
...
fig.add_trace(go.Scatter(
    ...
    y=mean_cases,
    mode='markers',
    name='Mean confirmed reported Cases/1M',
...
fig.update_layout(
    yaxis=dict(
        title="Causal Effect of Vaccination Coverage on confirmed reported Cases/1M",
        side="left",
        autorange=True,
        position=0.05,
        tickfont=dict(size=11)  # Smaller font for y-axis ticks
    ),
    ...
    yaxis2=dict(
        ...
        tickmode='auto',
        position=0.95,
        tickfont=dict(size=11)  # Smaller font for y-axis2 ticks
    ),
    ...
    yaxis3=dict(
        ...
        tickmode='auto',
        position=1,
        tickfont=dict(size=11)  # Smaller font for y-axis3 ticks
    ),
    ...
    xaxis=dict(
        ...
        tickfont=dict(size=11, family='Arial', color='black')  # Adjust x-axis font as needed
    ),
...
fig.write_html(fr"C:\MMR-EU-N-max1D-or-2D\C) {filename_plot_text} {year_range_text}.html")
fig.show()

print(fr"C:\MMR-EU-N-max1D-or-2D\C) {filename_plot_text} {year_range_text}.html has been saved.")
